...
*   Create a single-node GKE cluster with 8 L4 GPUs
*   Download Llama 2 7B model from Model Garden
*   Run a Kubernetes Job to fine-tune the model using L4 GPUs and store the model on Cloud Storage
...
```bash
git clone https://github.com/vmehmeri/generative-ai
cd generative-ai/fine-tuning/text/finetuning-llama-7b-on-l4
```
...
Create the GKE cluster with 8 L4 GPUs by running:
```bash
...
### Configuring GCS and service account with required permissions
...
### Download the model
Run the following to copy the Llama 2 7B model to the bucket created previously:
```bash
gsutil cp -r gs://vertex-model-garden-public-us-central1/llama2/llama2-7b-hf/ gs://$BUCKET/llama2-7b
```
> **Note: This artifact is provided only for Customer's use of Llama 2 and should not be shared with third parties.**

Once the copy operation has finished you can verify the model has been downloaded by running:
```bash
gcloud storage ls -l gs://$BUCKET_NAME/llama2-7b/
```

### Create a launch a fine-tuning job
